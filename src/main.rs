...
fn show_default(cfg: &Cfg) -> Result<()> {
	if let Some((toolchain, sysroot)) = try!(cfg.find_default()) {
		...
		println!("no default toolchain configured. run `multirost helpdefault`");
		Ok(())
	}
}
