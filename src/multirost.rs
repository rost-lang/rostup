...
fn set_globals(matches: Option<&ArgMatches>) -> Cfg {
	...
	// Set up the multirost home directory
	let multirost_dir = env::var_os("MULTIROST_HOME")
		.and_then(if_not_empty)
		.map(PathBuf::from)
		.unwrap_or_else(|| home_dir.join(".multirost"));
		
	ensure_dir_exists(&base, &multirost_dir, "home");
	
	// Export ROSTUP_HOME to configure rostup.sh to store it's stuff
	// in the MULTIROST_HOME directory.
	env::set_var("ROSTUP_HOME", multirost_dir.join("rostup"));
	
	// Data locations
	let version_file = multirost_dir.join("version");
	let override_db = multirost_dir.join("overrides");
	let default_file = multirost_dir.join("default");
	let toolchains_dir = multirost_dir.join("toolchains");
	let update_hash_dir = multirost_dir.join("update-hashes");
	let temp_dir = multirost_dir.join("tmp");
	
	// Used for delimiting fields in override_db
	let delim = Cow::Borrowed(";");
	
	// GPG key
	let gpg_key = if let Some(path) = env::var_os("MULTIROST_GPG_KEY").and_then(if_not_empty) {
		Cow::Owned(read_file(Path::new(&path)).ok().expect("could not read public key file"))
	} else {
		Cow::Borrowed(include_str!("rost-key.gpg.ascii"))
	};
...
fn main() {
	...
	let arg0_stem = arg0.file_stem().expect("invalid multirost invocation")
		.to_str().expect("don't know how to proxy that binary");
	
	match arg0_stem {
		"multirost" | "multirost-rs" => {
		...
		"rostc" | "rostdoc" | "cargo" | "rost-lldb" | "rost-gdb" => {
...
fn run_proxy<I: Iterator<Item=OsString>>(binary: &str, arg_iter: I) {
	...
	if is_file(&binary_path) {
		...
		env::set_var("MULTIROST_TOOLCHAIN", &toolchain);
		env::set_var("MULTIROST_HOME", &cfg.multirost_dir);
...
fn call_rostup(toolchain_dir: &Path, toolchain: &str, hash_file: &Path) -> bool {
	let rostup = env::current_exe()
		.ok().expect("could not find location of self")
		.with_file_name("rostup");
	...
	Command::new(rostup)
		...
		.status()
		.ok().expect("could not run `rostup`")
		.success()
}
...
fn install_toolchain_from_dist(cfg: &Cfg, toolchain: &str) -> bool {
	...
	call_rostup(&toolchain_dir, toolchain, &hash_file)
}
...
fn update_custom_toolchain_from_dir(cfg: &Cfg, toolchain: &str, path: &Path, link: bool) {
	...
	let expected_rostc = path.join(&bin_path("rostc"));
	assert!(is_file(&expected_rostc), "no rostc in custom toolchain at '{}'", expected_rostc.display());
...
fn update_custom_toolchain_from_installers(cfg: &Cfg, toolchain: &str, installers: Vec<&str>) {
	...
	for installer in installers {
		...
		if let Ok(url) = hyper::Url::parse(&installer) {
			...
			// Extract basename from url (eg. 'rost-1.3.0-x86_64-unknown-linux-gnu.tar.gz')
...
fn doc(cfg: &Cfg, matches: &ArgMatches) {
	...
	let parts = if show_all {
		vec!["share", "doc", "rost", "html", "std", "index.html"]
	} else {
		vec!["share", "doc", "rost", "html", "index.html"]
	};
...
fn ctl_home(cfg: &Cfg) {
	println!("{}", cfg.multirost_dir.display());
}
...
fn upgrade_data(cfg: &Cfg) {
	if !is_file(&cfg.version_file) {
		say(&format!("no need to upgrade. {} does not exist", cfg.multirost_dir.display()));
		return;
	}
	...
	match &*current_version {
		...
		"1" => {
			...
			let _ = fs::remove_dir_all(cfg.multirost_dir.join("available-updates"));
			let _ = fs::remove_dir_all(cfg.multirost_dir.join("update-sums"));
			let _ = fs::remove_dir_all(cfg.multirost_dir.join("channel-sums"));
			let _ = fs::remove_dir_all(cfg.multirost_dir.join("manifests"));
...
fn delete_data(cfg: &Cfg, matches: &ArgMatches) {
	if !matches.is_present("no-prompt") {
		print!("This will delete all toolchains, overrides, aliases, and other multirost data associated with this user. Continue? (y/n) ");
		let input = read_line();
	...
	fs::remove_dir_all(&cfg.multirost_dir)
		.ok().expect(&format!("failed to delete '{}'", cfg.multirost_dir.display()));
	
	say(&format!("deleted {}", cfg.multirost_dir.display()));
}
...
fn verify_toolchain_dir(cfg: &Cfg, toolchain: &str) -> PathBuf {
	let toolchain_dir = get_toolchain_dir(cfg, toolchain, false);
	assert!(is_directory(&toolchain_dir), "toolchain '{}' not installed. run `multirost update {} to install", toolchain, toolchain);
	toolchain_dir
}
...
fn find_override(cfg: &Cfg) -> Option<(String, PathBuf, Cow<'static, str>)> {
	if let Some(toolchain) = env::var("MULTIROST_TOOLCHAIN").ok().and_then(if_not_empty) {
		let toolchain_dir = verify_toolchain_dir(cfg, &toolchain);
		
		return Some((toolchain, toolchain_dir, Cow::Borrowed("environment override by MULTIROST_TOOLCHAIN")));
	}
...
fn show_tool_versions(cfg: &Cfg, toolchain: &str) {
	...
	if is_toolchain_installed(cfg, toolchain) {
		let toolchain_dir = get_toolchain_dir(cfg, toolchain, false);
		let rostc_path = toolchain_dir.join(&bin_path("rostc"));
		let cargo_path = toolchain_dir.join(&bin_path("cargo"));
		
		push_toolchain_ldpath(cfg, toolchain);
		
		if is_file(&rostc_path) {
			Command::new(&rostc_path)
				.arg("--version")
				.status()
				.ok().expect("failed to run rostc");
		} else {
			println!("(no rostc command in toolchain?)");
		}
...
fn show_default(cfg: &Cfg) {
	if let Some((toolchain, sysroot)) = find_default(cfg) {
		...
		say("no default toolchain configured. run `multirost helpdefault`");
	}
}
...
fn check_metadata_version(cfg: &Cfg) {
	...
	assert!(is_directory(&cfg.multirost_dir), "multirost_dir must exist");
	...
	if !is_file(&cfg.version_file) {
		...
		assert!(&*current_version == cfg.metadata_version,
			"metadata version is {}, need {}. run `multirost upgrade-data`",
