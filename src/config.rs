	...
	fn fmt(&self, f: &mut fmt::Formatter) -> ::std::result::Result<(), fmt::Error> {
		match *self {
			OverrideReason::Environment =>
				write!(f, "environment override by MULTIROST_TOOLCHAIN"),
...
impl Cfg {
	pub fn from_env(notify_handler: NotifyHandler) -> Result<Self> {
		...
		try!(utils::ensure_dir_exists("home", &multirost_dir, &notify_handler));
		...
		// GPG key
		let gpg_key = if let Some(path) = env::var_os("MULTIROST_GPG_KEY").and_then(utils::if_not_empty) {
		...
		let env_override = env::var("MULTIROST_TOOLCHAIN")
	...
	pub fn call_rostup(&self, toolchain_dir: &Path, toolchain: &str, hash_file: &Path) -> Result<()> {
		let rostup = env::current_exe()
			.ok().expect("could not find location of self")
			.with_file_name("rostup");
		
		let mut cmd = Command::new(rostup);
		cmd
	...
	pub fn install_toolchain_from_dist(&self, toolchain: &str) -> Result<()> {
		...
		self.call_rostup(&toolchain_dir, toolchain, &hash_file)
	}
	...
	pub fn update_custom_toolchain_from_dir(&self, toolchain: &str, path: &Path, link: bool) -> Result<()> {
		...
		let expected_rostc = path.join(&self.bin_path("rostc"));
		try!(utils::assert_is_file(&expected_rostc));
	...
	pub fn upgrade_data(&self) -> Result<bool> {
		...
		match &*current_version {
			...
			"1" => {
				...
				let _ = fs::remove_dir_all(self.multirost_dir.join("available-updates"));
				let _ = fs::remove_dir_all(self.multirost_dir.join("update-sums"));
				let _ = fs::remove_dir_all(self.multirost_dir.join("channel-sums"));
				let _ = fs::remove_dir_all(self.multirost_dir.join("manifests"));
	...
	pub fn delete_data(&self) -> Result<()> {
		if utils::path_exists(&self.multirost_dir) {
			utils::remove_dir("home", &self.multirost_dir, &self.notify_handler)
		} else {
			Ok(())
		}
	}
	...
	pub fn check_metadata_version(&self) -> Result<bool> {
		try!(utils::assert_is_directory(&self.multirost_dir));
		...
		self.with_path_var("LD_LIBRARY_PATH", &lib_path, || {
			self.with_path_var("DYLD_LIBRARY_PATH", &lib_path, || {
				self.with_default_var("CARGO_HOME", cargo_path.as_ref(), || {
					self.with_var("MULTIROST_TOOLCHAIN", toolchain.as_ref(), || {
						self.with_var("MULTIROST_HOME", self.multirost_dir.as_ref(), || {
							f()
						})
					})
				})
